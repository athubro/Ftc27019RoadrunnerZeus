...
public class StorageWLoader {
    public static class Params {
        ...
        public static double frontDisCheck = 3.6; //3.7--->3.4-->3.85-->3.6
        public static double middleDisCheck = 4.3;//change from 5 to 4.5 ->4.6-->4.3
        public static double backGreenRatio = 3.0;
        public static double middleGreenRatio = 2.6; //2.7-->2.6
       ...
       public static double Front_GreenRedRatio_LowerBound_PurpleBall = -0.03;//0.015
        ...
        //---------------------------------------------------middle Boundaries---------------------------------- all 0.05-->0.06
        public static double Middle_GreenRedRatio_UpperBound_PurpleBall = 0.06; //0.03->0.04
        //public static double boundaryUPPurple = 0.03;
        public static double Middle_GreenRedRatio_LowerBound_PurpleBall = -0.06; //0.03->0.04

        public static double Middle_BlueRedRatio_UpperBound_PurpleBall = 0.06;

        public static double Middle_BlueRedRatio_LowerBound_PurpleBall = -0.06;

        public static double Middle_GreenRedRatio_UpperBound_GreenBall = 0.06;
        //public static double boundaryUPPurple = 0.03;
        public static double Middle_GreenRedRatio_LowerBound_GreenBall = -0.06;

        public static double Middle_BlueRedRatio_UpperBound_GreenBall = 0.06;

        public static double Middle_BlueRedRatio_LowerBound_GreenBall = -0.06;
    ...
    public double rpmFieldOffset=1.0;
    private double timeCap = 1.2; //change from 1.5 to 1.2 s
    ...
    public boolean pastAutoTime = false;
...
//all turrets have been changed to turret with servo mode. if change back to continuous mode of pid control, search for "withservomode"
    ...
    public void update() {
        ...
        if (b.equals("N")) count++;
        if (count == 0) {
            myTurret.rpmOffsetFactor = 1.02*rpmFieldOffset;

        } else if (count == 1) {
            myTurret.rpmOffsetFactor = 1.011*rpmFieldOffset;
        } else if (count == 2) {
            myTurret.rpmOffsetFactor = 1*rpmFieldOffset;
        } else {
            myTurret.rpmOffsetFactor = 1*rpmFieldOffset;
        }
        timeRN=timer.time();
...
public void color (double rgb) {
    ...
    public void setFrontUp() {
        ...
        double t =generalTimer.seconds();
        frontLoaderNextTrigger=t;
        frontLoaderNextPos=1;
        //frontKick.setPosition(1);
    }
    public void setMiddleUp() {
        double t =generalTimer.seconds();
        midLoaderNextTrigger=t;
        midLoaderNextPos=1; }
    public void setBacUp() {
        double t =generalTimer.seconds();
    ...
    public void resetKick() {
        ...
        backLoaderNextTrigger=t;
        backLoaderNextPos=0;
        midLoaderNextTrigger=t+0.04; // 0.067->0.04
        midLoaderNextPos=0;
        frontLoaderNextTrigger=t+0.04*2; //0.067->0,04
        frontLoaderNextPos=0;
        gateNextTrigger=backLoaderNextTrigger;// changed from frontloadertrigger to backloadertrigger
        gateNextPos=gateMidPOs;
        changeFlagTrigger=frontLoaderNextTrigger+0.4 ;//chnaged from gatenextrigger to frontloadertrigger
        nextFlagValue=true;
    }
    ...
    public  void loadOne(){
        ...
        String midColor;
        String frontColor;
        backColor=ballArray[2];
        midColor=ballArray[1];
        frontColor=ballArray[0];
        if (!backColor.equals("N")){
            flagLoadingBack=true;
        } else if (!midColor.equals("N")) {
            flagLoadingMiddle=true;
        } else if (!frontColor.equals("N")) {
            flagLoadingFront=true;
        }
