...
public final class Turret {
    public class Params {
        ...
        public double kP = 50; //31.35->38-->
        public double kI = 0;
        public double kD = 0.0;
        public double kF = 10;

        public double toleranceRPM = 70;
        ...
        public static final double MIN_TURRET_POWER = 0.03; //change from 0.05->0.03
        public static final double KP_TURRET = 0.015;
        public static final double SEARCH_POWER = 0.08; //change from 0.1-> 0.08
    ...
    // Hardware
    public final Servo turretAim;          // continuous servo for yaw control
    ...
    public double rpmOffsetFactor = 1;
    public double shotDetectLastTime = 0;
    public boolean shotDetected = false;
//servo mode ----------
    public double turretPos = 0.5;
    double posPerDegree = 0.0123456790123457;
    double aimTolerance = 0.1; //in degrees
    ...
    public void setTrackingMode(boolean input) {
        if (input){
            this.adjustAiming=true;

        } else{
            this.adjustAiming=false;
        }
        this.trackingMode = input;
    }
    ...
    private void pidUpdate() {
        ...
        double targetVelocity = (rpmOffsetFactor*targetRPM / 60.0) * Params.TICKS_PER_REV;
        ...
        double errorLeft = Math.abs(rpmOffsetFactor*targetRPM - currentRPMLeft);
        double errorRight = Math.abs(rpmOffsetFactor*targetRPM - currentRPMRight);
    ...
    public void shotDetection_backup(){ // old method, don't use
    ...
    public void shotDetection(){
        ...
        if ( !(lastLeftRPM == 0 || lastRightRPM == 0))
        {
            ...
            if (deltaLeftRPM < - 400 && deltaRightRPM < - 400) {
                shotDetected = true;
                lastShotTimer = timer.time();

            } else {

                if (timer.time() - lastShotTimer > 0.5 && shotDetected) {
                    shotDetected = false;
                }
            ...
            if (result != null && result.isValid()) {
                for (LLResultTypes.FiducialResult fid : result.getFiducialResults()) {
                    if (fid.getFiducialId() == PARAMS.TARGET_TAG_ID) {
                        ...
                        //---new---
                        if (disToAprilTag > 75 && PARAMS.TARGET_TAG_ID == 20) {
                            errorAngleDeg = fid.getTargetXDegrees() - (targetAngle+2);
                        } else if (disToAprilTag > 75 && PARAMS.TARGET_TAG_ID == 24) {
                            errorAngleDeg = fid.getTargetXDegrees() - (targetAngle-2);
                        } else {
                            errorAngleDeg = fid.getTargetXDegrees() - targetAngle;
                        }
...
//----------------servo Mode
    ...
    public void updateTurretControlV2() {
        ...
        if (trackingMode) {
            ...
            if (tagFound && adjustAiming) {
                if (Math.abs(errorAngleDeg) < aimTolerance) {
                    errorAngleDeg = 0;
                }
                turretPos -= errorAngleDeg * posPerDegree;
                turretPos = clamper(turretPos, 0, 1);
                turretAim.setPosition(turretPos);
                adjustAiming=false;  //adjusting aiming once only;
            } else {
                color(0.28);
            }
            ...
            tagFound = false;
          // manual control — direct feedthrough of the stored joystick value
          //turretPos -= manualTurretInput;
        }


          telemetryData.tagFound = tagFound;
          telemetryData.errorAngleDeg = errorAngleDeg;
          telemetryData.turretAimPos = turretPos;

    }


    public void updateTurretControlV2_backup() {
    ...
    public void turretSetPos () {


        turretAim.setPosition(turretPos);
    }

    public void turretManualPos(double pos) {
        turretPos = pos;
        turretAim.setPosition(turretPos);
    }
    ...
    public void calcTargetAngleSpeed() {
        ...
        if (tagFound) {
            if (x<75) {
                ...
                shooterAngleSetting = -0.0000046*x*x*x+0.00108*x*x-0.0885*x+2.54; //new formula Jan 4
            } else{
            ...
            if (!constantRPM){
               ...
               targetRPM =0.00284*x*x*x-0.343*x*x+23.8*x+2022; //new formula Jan 4
               //targetRPM = 0.0101*x*x*x-1.63*x*x+90.7*x+964;
    ...
    public void calcTargetAngleSpeed_backup() {  //old one, dont use
        double x = disToAprilTag;
        double shooterAngleSetting;
        double shooterAngle;
        if (tagFound) {
